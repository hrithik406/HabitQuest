...
router.post("/buy", async (req, res) => {
    try {
        ...
        const item = item_1.SHOP_ITEMS.find((i) => i.id === itemId);
        if (!item) {
        ...
        const alreadyOwned = user.ownedRewards.some((r) => r.itemId === itemId);
        if (alreadyOwned) {
        ...
        if (user.stats.totalGoldSpent >= 1000) {
            const hasEcoSpend = user.unlockedAchievements.some((a) => a.achievementId === "eco_spend");
            if (!hasEcoSpend) {
        ...
        res.status(500).json({ error: "Server error processing transaction", details: err.message });
    }
});
// ─────────────────────────────────────────────────────────────────
...
router.post("/equip", async (req, res) => {
    try {
        ...
        const ownsItem = user.ownedRewards.some((r) => r.itemId === itemId);
        if (!ownsItem) {
        ...
        if (category === "theme") {
            ...
            const inventoryIndex = user.ownedRewards.findIndex((r) => r.itemId === itemId);
            if (inventoryIndex > -1) {
                user.ownedRewards.splice(inventoryIndex, 1);
            }
            // 2. STACK THE DURATION
            const existingPowerup = user.activePowerups.find((p) => p.itemId === itemId);
            if (existingPowerup) {
        ...
        res.status(500).json({ error: "Server error equipping item", details: err.message });
    }
});
