...
const achievementsEngine_1 = require("../utils/achievementsEngine");
 ...
 */
function computeRewards(baseXp, baseGold, streak) {
 ...
 */
async function runHabitComplete(habit, currentDate, userId) {
    ...
    const now = new Date();
    user.activePowerups = user.activePowerups.filter((p) => new Date(p.expiresAt) > now);
    ...
    const hasShield = user.activePowerups.some((p) => p.itemId === "streak_shield");
    if (lastCompletedDate === "") {
        ...
        user.activePowerups = user.activePowerups.filter((p) => p.itemId !== "streak_shield");
        // ⬇️ ADD THIS NEW BLOCK: Give them the Close Call achievement! ⬇️
        const hasEcoShield = user.unlockedAchievements.some((a) => a.achievementId === "eco_shield");
        if (!hasEcoShield) {
    ...
    const hasXpBoost = user.activePowerups.some((p) => p.itemId === "xp_boost");
    if (hasXpBoost) {
    ...
    const { newlyUnlocked } = (0, achievementsEngine_1.evaluateAchievements)(user, newStreak);
...
router.get("/", async (req, res) => {
    try {
        ...
        const user = await (0, ensureUser_1.ensureUser)(userId);
        const currentDate = (0, date_1.getDateIsoInTimeZone)(user.timezone);
        const yesterday = getYesterday(currentDate); // ⬅️ Get yesterday's date using your helper function
        const habits = await Habit_1.default.find({ userId, isArchived: false }).sort({ createdAt: 1 });
...
router.post("/:id/complete", async (req, res) => {
    try {
        ...
        const user = await (0, ensureUser_1.ensureUser)(userId);
        const currentDate = (0, date_1.getDateIsoInTimeZone)(user.timezone);
        const habit = await Habit_1.default.findOne({ _id: req.params.id, userId });
        if (!habit) {
            res.status(404).json({ error: "Habit not found" });
            return;
        }
        if (habit.isArchived) {
            res.status(400).json({ error: "Cannot complete an archived habit" });
            return;
        }
        try {
...
router.post("/:id/undo", async (req, res) => {
    try {
        const { userId } = req.body;
        const user = await User_1.default.findById(userId); // Or ensureUser(userId)
...
// ─────────────────────────────────────────────────────────────────
router.put("/:id/milestone/:milestoneId", async (req, res) => {
    try {
        const { userId } = req.body;
        if (!userId) {
            res.status(400).json({ error: "userId is required" });
        ...
        if (!mongoose_1.default.isValidObjectId(req.params.milestoneId)) {
        ...
        if (habit.isArchived) {
            res.status(400).json({ error: "Cannot modify an archived habit" });
            return;
        }
...
router.patch("/:id", async (req, res) => {
    try {
        const { userId, title, description, icon, color, frequency, customDays, baseXp, baseGold, milestones } = req.body;
        const update = { title, description, icon, color, frequency, customDays, baseXp, baseGold };
        if (milestones !== undefined) {
            update.milestones = milestones.map((m) => ({ title: m.title, isCompleted: false }));
        }
        const habit = await Habit_1.default.findOneAndUpdate({ _id: req.params.id, userId }, update, { new: true, runValidators: true });
        if (!habit) {
            res.status(404).json({ error: "Habit not found" });
            return;
        }
...
router.delete("/:id", async (req, res) => {
    try {
        const { userId } = req.query;
        const habit = await Habit_1.default.findOneAndUpdate({ _id: req.params.id, userId }, { isArchived: true }, { new: true });
        if (!habit) {
            res.status(404).json({ error: "Habit not found" });
            return;
        }
